...
class Excercise {
    ...
    get totalYears() {
        return inventors.reduce((a, c) => a + (c.passed - c.year), 0, 0)
    }

    get sortByLivingYears() {
        return inventors.sort((a, b) => (a.passed - a.year) - (b.passed - b.year))
    }

    get searchBoulvards() {
        return boulevards.filter(i => i.includes('de'))
    }
    ...
    get sortByLastName() {
        return people.sort((a, b) => this.lastName(a) > this.lastName(b) ? 1 : -1)
    }
...
const ex = new Excercise();
console.log(ex.instanceSum)
